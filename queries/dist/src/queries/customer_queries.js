...
export async function fetchActiveCustomers(db, daysInactive = 90) {
...
export async function findCustomersBySegment(db, segmentName) {
    const query = `
    SELECT 
        c.customer_id,
        c.email,
...
export async function getCustomerProfile(db, customerId) {
  ...
  `;
    ...
    const addresses = (await db.all(addressesQuery, [customerId]));
    // Get order count
    const orderCountQuery = "SELECT COUNT(*) as order_count FROM orders WHERE customer_id = ?";
    // Await the promise returned by db.get
...
export async function searchCustomersByName(db, firstName, lastName) {
    let query = `
    SELECT 
        c.*,
        c.status,
...
export async function listCustomersWithReviews(db) {
    const query = `
    SELECT 
        c.*,
