export async function getShippingAddresses(db, customerId) {
    const query = `
    SELECT 
        sa.address_id,
        sa.street,
...
export async function findOrdersByDestination(db, state) {
    const query = `
    SELECT 
        o.order_id,
    ...
    `;
    const rows = await db.all(query, [state]);
    return rows;
}
export async function getUnshippedOrders(db) {
    const query = `
...
export async function calculateShippingCostsByState(db) {
    const query = `
...
export async function findDeliveryDelays(db, expectedDays = 5) {
    const query = `
    SELECT 
        o.order_id,
        o.created_at as order_date,
    ...
    `;
    const rows = await db.all(query, [expectedDays]);
    return rows;
}
