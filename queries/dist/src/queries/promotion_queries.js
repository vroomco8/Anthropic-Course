...
export async function checkPromoEligibility(db, customerId, promoCode) {
    const query = `
    WITH promo_info AS (
        SELECT
            promotion_id,
...
export async function findExpiringPromotions(db, days = 7) {
...
export async function getPromotionPerformance(db, promoId) {
    const query = `
...
export async function findUnusedPromotions(db) {
    const query = `
