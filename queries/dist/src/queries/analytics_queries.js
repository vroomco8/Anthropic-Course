export async function calculateCustomerLifetimeValue(db, customerId) {
    const query = `
    SELECT
        c.id,
        c.email,
    ...
    GROUP BY c.id
    `;
    const result = await db.get(query, [customerId]);
    if (!result)
        return {};
    const categoryQuery = `
      SELECT
      ...
      LIMIT 5
      `;
    const categories = await db.all(categoryQuery, [customerId]);
...
export async function getSalesByCategory(db, startDate, endDate) {
    const query = `
    SELECT
    ...
    `;
    const categories = await db.all(query, [startDate, endDate]);
    const results = [];
    for (const category of categories) {
        ...
        LIMIT 1
        `;
        const topProduct = await db.get(topProductQuery, [
        ...
        `;
        const segments = await db.all(segmentQuery, [
            category.category_id,
            startDate,
            endDate,
        ]);
...
export async function findRepeatCustomers(db, minOrders = 2) {
    const query = `
    ...
    `;
    const customers = await db.all(query, [minOrders]);
    const results = customers.map((customer) => {
        ...
        if (orderDates.length > 1) {
            ...
            const totalDays = (dates[dates.length - 1].getTime() - dates[0].getTime()) /
                (1000 * 60 * 60 * 24);
...
export async function getProductPerformance(db, productId) {
    const productQuery = `
    SELECT
        p.id,
        p.name,
    ...
    const reviewQuery = `
      ...
      WHERE product_id = ?
      `;
    const reviews = await db.get(reviewQuery, [productId]);
    const inventoryQuery = `
      ...
      WHERE product_id = ?
      `;
    const inventory = await db.get(inventoryQuery, [productId]);
    const turnoverQuery = `
      SELECT
      ...
      `;
    const turnover = await db.get(turnoverQuery, [productId]);
    const currentStock = inventory?.current_stock || 0;
    const sold90d = turnover?.quantity_sold_90d || 0;
    const inventoryTurnover = currentStock > 0 ? ((sold90d / 90) * 365) / currentStock : 0;
    const segmentQuery = `
      SELECT
          c.segment,
          COUNT(DISTINCT c.id) as customer_count,
      ...
      `;
    const segments = await db.all(segmentQuery, [productId]);
    ...
    const returnRateEstimate = reviews?.review_count > 0
...
export async function calculateSegmentMetrics(db, segmentName) {
    const segmentQuery = `
    SELECT
        COUNT(DISTINCT c.id) as customer_count,
    ...
    WHERE c.segment = ?
    `;
    const segmentStats = await db.get(segmentQuery, [segmentName]);
    if (!segmentStats || segmentStats.customer_count === 0) {
        return { segment: segmentName, customer_count: 0 };
    }
    const avgCustomerValue = segmentStats.customer_count > 0
    ...
    const topProductsQuery = `
      ...
      LIMIT 10
      `;
    const topProducts = await db.all(topProductsQuery, [segmentName]);
    ...
    const shippingQuery = `
      ...
      LIMIT 5
      `;
    const shippingStates = await db.all(shippingQuery, [segmentName]);
    ...
    return {
        ...
        orders_per_customer: Math.round((segmentStats.orders_per_customer || 0) * 100) / 100,
...
export async function findTrendingProducts(db, days = 30) {
    ...
    const results = products.map((product) => {
        ...
        const newCustomerRatio = totalCustomers > 0 ? product.new_customers / totalCustomers : 0;
        return {
