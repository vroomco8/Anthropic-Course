export async function getWarehouseInventory(db, warehouseId) {
    const query = `
    SELECT 
        i.product_id,
    ...
    `;
    const rows = await db.all(query, [warehouseId]);
    return rows;
}
export async function checkProductAvailability(db, productId) {
    const query = `
    SELECT 
        i.warehouse_id,
...
export async function findStockTransfersNeeded(db) {
    const query = `
...
export async function getInventoryValueByWarehouse(db) {
    const query = `
    SELECT 
        w.warehouse_id,
        w.name AS warehouse_name,
...
export async function fetchReservedInventory(db) {
    const query = `
...
export async function getInventoryMovements(db, days = 30) {
    const query = `
